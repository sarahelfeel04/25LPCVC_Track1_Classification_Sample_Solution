...
# Basic preprocessing without histogram equalization or CLAHE
...
class PreservedMobileNetV2(nn.Module):
    def __init__(self, weights_path=None):
        ...
        # Initialize with 64 classes to match COCO weights
        self.model = models.efficientnet_b0(weights=False)  # Initialize without pretrained weights
        self.model.classifier[1] = nn.Linear(self.model.classifier[1].in_features, 64) # Modify classifier
        ...
        if weights_path and os.path.exists(weights_path):
            ...
            # Modify the pretrained dictionary to fit the current model
...
pretrained_path = "model/efficientnet-b0-355c32eb.pth"
...
def fine_tune_model(model, data_dir, file_names, ground_truth, epochs=5):
    ...
    for name, param in model.model.named_parameters():
        if 'classifier' in name:  # Only fine-tune classifier
            param.requires_grad = True
        else:
            param.requires_grad = False

    # Use smaller learning rate for fine-tuning
    optimizer = torch.optim.Adam(model.model.classifier.parameters(), lr=0.001)
...
# Create new dataset with basic preprocessing
preprocessor = BasicPreprocessor()
input_tensors = []
sample_paths = []
...
if input_tensors:
    ...
    compile_job = qai_hub.submit_compile_job(
        pt_model,
        name="basic_preserved_classifier_mobilenetv2",
    ...
    print("Creating dataset with basic preprocessing...")
    processed_images = []
    ...
    if len(predicted_classes.shape) == 2:
        ...
        with open("model_performance.txt", "a") as f:
            ...
            f.write(f"Model: Basic Preserved Classifier + Fine-tuned MobileNetV2\n")
