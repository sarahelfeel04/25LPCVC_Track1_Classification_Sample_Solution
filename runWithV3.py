...
# Custom wrapper class for preprocessing and MobileNetV3-Small
import torch
from torchvision import models, transforms
import torch.nn as nn
from PIL import Image

class PreprocessedMobileNetV3(nn.Module):
    def __init__(self, num_classes, pretrained_weights_path):
        super(PreprocessedMobileNetV3, self).__init__()
        
        # Load MobileNetV3 Small model
        self.mobilenet_v3 = models.mobilenet_v3_small(pretrained=False)
        
        # Load the pretrained weights
        state_dict = torch.load(pretrained_weights_path, map_location=torch.device('cpu'))
        self.mobilenet_v3.load_state_dict(state_dict, strict=False)  # Allow partial load
        
        # Replace the classifier to match the required number of classes
        self.mobilenet_v3.classifier[3] = nn.Linear(in_features=1024, out_features=num_classes)
    ...
    def forward(self, img):
        return self.mobilenet_v3(img)


# Step 1: Upload Dataset
try:
    dataset_id = "dv74qmp02"  # Manually set dataset ID
...
# Step 2: Load Model
num_classes = 64
pretrained_path = "./model/mobilenet_v3_small.pth"  # Update path to match MobileNetV3 weights
model = PreprocessedMobileNetV3(num_classes=num_classes, pretrained_weights_path=pretrained_path)
model.eval()
...
try:
    ...
    with open("accuracy_log2.txt", "w") as f:
